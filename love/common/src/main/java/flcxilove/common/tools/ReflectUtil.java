...
public class ReflectUtil {
   ...
   * @return 萃取对象字段列表
   */
  public static List<Field> getFieldsWithAnnotation(Class target, Class... annotationCls) {
    ...
    List<Field> fields = Arrays.stream(target.getDeclaredFields())
        ...
        .filter(
            ...
            field -> Arrays.stream(annotationCls).filter(cls -> field.getAnnotation(cls) != null).count() > 0)
        // 数组转列表
  ...
  /**
   * 调用对象成员方法，无视修饰符（priavte,protected）
   *
   * @param object 数据对象
   * @param methodName 方法名
   * @param parameterTypes 方法参数类型
